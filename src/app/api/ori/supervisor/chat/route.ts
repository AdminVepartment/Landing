...
export async function POST(req: NextRequest) {
  ...
  const { message, department, domain, sessionId } = body as {
    message: string;
    department: string;
    domain?: string;
    sessionId: string;
  ...
  if (!department?.trim()) {
    return NextResponse.json(
      { output: "No department specified.", expression: "curious" },
      { status: 400 },
    );
  }

  try {
    const response = await fetch(`${CREWAI_URL}/supervisor/chat`, {
      method: "POST",
    ...
    return NextResponse.json(
      {
        output: `Couldn't reach the backend. Is it running on ${CREWAI_URL}? (${errorMessage})`,
        expression: "curious",
      },
      { status: 502 },
    );
